...
const App = ({navigation, route}) => {
  ...
  const apiCall_product = async () => {
    ...
    var response = await Helper.POST(Urls.booking_extra_product_wise, formdata);
    if (response.status == 200) {
      console.log('product_id>', product_id);
      console.log('eee>', response);
      setLoding(false);
      if (validationempty(response.booking_extra_product)) {
        var votes = [];
        let qty = 1;
        for (
          let index = 0;
          index < response.booking_extra_product.length;
          index++
        ) {
          const elementpr = response.booking_extra_product[index];
          votes.push({
            id: elementpr.id,
            product_id: elementpr.product_id,
            name: elementpr.services_name,
            productName: elementpr.productName,
            services_per_price: elementpr.services_per_price,
            services_per_point: elementpr.services_per_point,
  ...
  const apiCall_Add_Cart = async () => {
    if (checked.length > 0) {
      ...
      var response = await Helper.POST(
        Urls.booking_add_cart_extra_details,
        formdata,
      );
      if (response.status == 200) {
                          ...
                          <View>
                            <View style={{flexDirection: 'row', marginTop: 6}}>
                              ...
                              <Text
                                ...
                                {item.services_per_price} {item.currency_code}
                              </Text>
