...
const DealList = ({navigation, route}) => {
  const {flag, voucher_id, business_id, bussLogo} = route.params;
  ...
  const apiCallLanguage_Info = async () => {
    ...
    formdata.append('screen_name', 'Campaigns');
  ...
  const apiCalllist = async () => {
    ...
    var response = await Helper.POST(Urls.get_vouchers_campaign, formdata);
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => {
                        if (flag == 'lottery') {
                          navigation.navigate('LotteryInfo', {
